...
from worker.utils.process_utils import store_process_id, kill_process_group
...
class JobExecutor:
    ...
    def __init__(self, scheduler: Optional[ResourceScheduler] = None) -> None:
        """
        初始化作业执行器
    ...
    def _wait_for_scheduling(
        ...
        Args:
            job_id: 作业ID
            timeout: 最大等待时间（秒），默认从配置读取
        
        Returns:
            作业对象 或 None（被取消时）
        
        Raises:
            ValueError: 作业不存在或状态异常
            TimeoutError: 等待超时
        """
        if timeout is None:
            timeout = self.config.JOB_SCHEDULING_TIMEOUT
    ...
    def _run_job(self, job: Job) -> int:
        ...
        try:
            with (
                ...
                # 记录进程ID
                store_process_id(job.id, process.pid)
                ...
                try:
                    ...
                    kill_process_group(
                        process.pid, 
                        grace_period=self.config.PROCESS_TERM_GRACE_PERIOD
                    )
                    exit_code = -1
