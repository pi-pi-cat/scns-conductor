...
class ResourceManager:
    ...
    def can_allocate(self, cpus: int) -> bool:
        ...
        """
        with self._lock:
            return self._can_allocate_unlocked(cpus)

    def _can_allocate_unlocked(self, cpus: int) -> bool:
        """
        检查是否可以分配请求的CPU数量（内部使用，不加锁）
    ...
    def _get_metrics_unlocked(self) -> ResourceMetrics:
        ...
        utilization = (
            (self._used_cpus / self._total_cpus * 100) if self._total_cpus > 0 else 0.0
        )
